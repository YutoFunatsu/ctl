...
%  一つのfigオブジェクト中に任意の縦ｎ個のサブプロットを行い各プロットに重ねてプロットするものがあれば行う関数mプログラム(makeParallelFigs.m)
%
...
%       3:labelName
%           型：文字ベクトルが入ったセル配列
%           内容：各サブプロットのラベル名を1個x,y、2個目y軸,３個目y軸・・・の順番に格納したテキスト
%       4:Plotdata
%           型：プロットする数値データが入った行列
...
function displayFigs(figAmount,titleName,labelName,Plotdata,addPlotdata,supTitleName)
    ...
    % Figureオブジェクト新規作成
    figure('Name',supTitleName)
    sgtitle(supTitleName)                      % 図全体のタイトルを描画
    ...
    for index = 1:1:figAmount   
        ...
        ylabel(labelName(index + 1))                    % 対応するサブプロットのy軸の名前を描画
        grid on                                     % ます目の表示
